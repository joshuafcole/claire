...
module.exports = function(window, localRoot) {
  ...
  claire.init = function() {
    ...
    document.querySelector(container + ' > .content').appendChild(claire.claire);
  };
  ...
  claire.show = function() {
    var opened = ltrap.showContainer(container);
    if(opened) {
      // Closing the console deletes claire?
      if(!document.querySelector('#claire')) {
        document.querySelector(container).appendChild(claire.claire);
      }
  ...
  claire.openMatch = function() {
    ...
    fs.appendFile(filepath, '', function(err) {
      ...
      ltrap.execCommand('tabs.goto', ltrap.toKeyword('last'));
      ltrap.execCommand('tabs.focus-active');
    });
  };
  ltrap.addCommand({
